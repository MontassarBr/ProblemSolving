...
, and (r+1,c+1)*/
    ...
    if (valid(r,c+1))
    {
        rec(r,c+1,s+cc[r][c+1]);
    }
    if (valid(r-1,c+1))
    {
        rec(r-1,c+1,s+cc[r-1][c+1]);
    }
    if (valid(r-1,c))
    {
        rec(r-1,c,s+cc[r-1][c]);
    }
    if (valid(r-1,c-1))
    {
        rec(r-1,c-1,s+cc[r-1][c-1]);
...
void solve() {
    ...
    res+=(max(s1.size(),s2.size())-1)*9;
    cout<<res<<endl;
    return;
    }
    a=0;
    while (s1[a]==s2[a] && a<s1.size())
    {
        a++;
    }
    if (a==s1.size())
    {
        ...
        cout<<abs(s1[a]-s2[a])+(s1.size()-a-1)*9<<endl;
    }
