...
bool can(ll md){
    ...
    for(int i=0;i<n;i++){
        if (i==0)
        {
            s+=max(0ll,min(max(vv2[i+1],md),md)-v[i]);
        }
        else if (i==n-1)
        {
            s+=max(0ll,min(max(vv[i-1],md),md)-v[i]);
        }
        else
        {
            l=min(vv[i-1],vv2[i+1]);
            s+=max(0ll,min(md,max(md,l))-v[i]);
        }
        if (s>k)
        {
            return false;
...
void solve() {
    ...
    if (cc[0][0]==cc[0][1] && cc[0][0]==cc[0][2] && cc[0][0]!='.')
    {
        cout<<cc[0][0]<<endl;
    }
    else if (cc[1][0]==cc[2][0] && cc[0][0]==cc[2][0]&& cc[1][0]!='.')
    {
        cout<<cc[1][0]<<endl;
    }
    
    else if (cc[0][0]==cc[1][1] && cc[0][0]==cc[2][2]&& cc[0][0]!='.')
    {
        cout<<cc[0][0]<<endl;
    }
      else if (cc[0][2]==cc[1][1] && cc[2][0]==cc[1][1]&& cc[0][2]!='.')
    {
        cout<<cc[0][2]<<endl;
    }
      else if (cc[0][1]==cc[1][1] && cc[0][1]==cc[2][1]&& cc[0][1]!='.')
    {
        cout<<cc[0][1]<<endl;
    }
      else if (cc[0][2]==cc[1][2] && cc[0][2]==cc[2][2]&& cc[0][2]!='.')
    {
        cout<<cc[0][2]<<endl;
    }
      else if (cc[0][0]==cc[0][1] && cc[0][0]==cc[0][2]&& cc[0][0]!='.')
    {
        cout<<cc[0][0]<<endl;
    }
      else if (cc[1][0]==cc[1][1] && cc[1][0]==cc[1][2]&& cc[1][0]!='.')
    {
        cout<<cc[1][0]<<endl;
    }
      else if (cc[2][0]==cc[2][1] && cc[2][0]==cc[2][2]&& cc[2][0]!='.')
    {
        cout<<cc[2][0]<<endl;
    }
    else
