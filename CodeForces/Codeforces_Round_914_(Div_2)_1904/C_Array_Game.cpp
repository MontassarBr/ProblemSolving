...
void solve() {
    ...
    for (int i = 0; i < n; i++)
    {
        if(lower_bound(all(v1),v[i])!=v1.end()){
            c=min(c,abs(v[i]-*lower_bound(all(v1),v[i])));
            if(lower_bound(all(v1),v[i])!=v1.begin()) c=min(c,abs(v[i]-*(lower_bound(all(v1),v[i])-1)));
        } 
        else
        {
            c=min(c,abs(v[i]-v1[v1.size()-1]));
        }
